...
def getIndicesOfGoodData(data):
    ...
    m = getMaskForBadKeplerData()
    idx = transitfunc.getIndicesOfGoodData(time, flux, flag, m)
    return np.where(idx)[0]


def getMaskForBadKeplerData():
...
def medianZero(data, col):
    return transitfunc.medianZero(data, col)
...
def markTransitCadences(time, period_days, epoch_bkjd, duration_days,\
    ...
    return transitfunc.markTransitCadences(time, period_days,
                                           epoch_bkjd,
                                           duration_days,
                                           numberOfDurations,
                                           flags)
