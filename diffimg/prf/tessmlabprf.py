...
from abstracttessprf import AbstractTessPrf
...
class TessMatlabPrf(AbstractTessPrf):
    ...
    * TODO: For speed, interpolate prfObjects before extracting out 13x13 regular arrays
    ...
    def getPrfAtColRow(self, col, row):
        ...
        col = float(col)
        row = float(row)
        sector, camera, ccd =  self.sector, self.camera, self.ccd
        ...
        key = "%1i-%1i-%02i" %(sector, camera, ccd)

        if key not in self.cache:
            self.cache[key] = self.readPrfFile(sector, camera, ccd)
    ...
    def getRegularlySampledPrfByOffset(self, singlePrfObj, colOffset, rowOffset):
        ...
        #TODO This conversion should happen when the prfObjects are loaded.
    ...
    def readPrfFile(self, sector, camera, ccd):
